...
impl From<Value> for CustomValue {
    fn from(value: Value) -> Self {
        CustomValue::Json(value)
    }
}

impl From<Thing> for CustomValue {
    fn from(value: Thing) -> Self {
        CustomValue::Thing(value)
    }
}

// Helper function to create a CustomValue from a JSON value
fn json(value: Value) -> CustomValue {
    CustomValue::Json(value)
}
...
impl Serialize for CustomValue {
    ...
    {
        match self {
            CustomValue::Json(v) => v.serialize(serializer),
            CustomValue::Thing(t) => t.serialize(serializer),
        }
    }
}
...
impl<'de> Deserialize<'de> for CustomValue {
        ...
        D: Deserializer<'de>,
    {
        ...
        if let Value::Object(map) = &value {
            if let (Some(Value::String(tb)), Some(Value::Object(id))) = (map.get("tb"), map.get("id")) {
                if let Some(Value::String(id)) = id.get("String") {
                    return Ok(CustomValue::Thing(Thing::from((tb.as_str(), id.as_str()))));
                }
            }
        }
        
        Ok(CustomValue::Json(value))
    }
}
...
impl fmt::Display for CustomValue {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            CustomValue::Json(v) => write!(f, "{}", v),
            CustomValue::Thing(t) => write!(f, "{}", t),
        }
    }
}

impl CustomValue {
   fn json<T: Into<serde_json::Value>>(value: T) -> Self {
        CustomValue::Json(value.into())
    }
    
    fn thing(tb: &str, id: &str) -> Self {
        CustomValue::Thing(Thing::from((tb, id)))
    }
}
...
impl Document {
    ...
    fn create_version(&mut self) -> u64 {
        ...
        self.changes.push(Change {
            ...
            new_data:  HashMap::from([
                ("version".to_string(), CustomValue::json(version)),
                // ("snapshot".to_string(), CustomValue::json(snapshot)),
            ]),
        });
        version
    }
    ...
    //         let local_overrides: HashMap<String, CustomValue> = block
    //             .data
    //             .iter()
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    doc.add_block(
        "header",
        HashMap::from([
            ("text".to_string(),  CustomValue::json("My First Blog Post")),
            ("level".to_string(),  CustomValue::json((1))),
        ]),
    );
    doc.add_block(
        "paragraph",
        HashMap::from([(
            "text".to_string(),
             CustomValue::json("This is the content of my first blog post."),
        )]),
    );
    ...
    let mut author_bio_data: HashMap<String, CustomValue> = HashMap::new();

    author_bio_data.insert("name".to_string(), json(Value::String("John Doe".to_string())));
    author_bio_data.insert("bio".to_string(), json(Value::String("Default bio for John Doe".to_string())));
    author_bio_data.insert("image_url".to_string(), json(Value::String("https://example.com/johndoe.jpg".to_string())));
    author_bio_data.insert("social_links".to_string(), json(serde_json::json!([
        {"platform": "Twitter", "url": "https://twitter.com/johndoe"},
        {"platform": "LinkedIn", "url": "https://linkedin.com/in/johndoe"}
    ])));
    author_bio_data.insert("display_config".to_string(), json(serde_json::json!({
        "name": true,
        "bio": true,
        "image_url": false,
        "social_links": true
    })));
    author_bio_data.insert("is_attached".to_string(), json(Value::Bool(true)));

    // Using Thing directly for record links
    author_bio_data.insert("template_id".to_string(), CustomValue::Thing(Thing::from(("component_templates", "ct-001"))));
    author_bio_data.insert("friends".to_string(), json(serde_json::json!([
        Thing::from(("person", "tobie")),
        Thing::from(("person", "simon"))
    ])));
    ...
    author_bio_data.insert(
        ...
        CustomValue::json({
            "This is a custom bio for John Doe, overriding the template."
        }),
    );
    ...
    // let _doc_with_attached = store.get_document(&doc_id_with_attached).await?.unwrap();
